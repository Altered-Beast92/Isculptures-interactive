...
export default function SiteNav() {
  ...
  return <header className="site-header"><nav aria-label="Main navigation" onKeyDown={event => { if (event.key === 'Escape' && open) { setOpen(false); toggle.current?.focus(); } }}>
    ...
    <div className={'site-links' + (open ? ' is-open' : '')} id="site-links">
      <a href="/services" aria-current={pathname.startsWith("/services") || (pathname.startsWith("/pages/") && pathname !== "/pages/about") ? "page" : undefined}>Services</a><a href="/work" aria-current={pathname.startsWith("/work") ? "page" : undefined}>Our work</a><a href="/guides" aria-current={pathname.startsWith("/guides") ? "page" : undefined}>Guides</a><a href="/pages/about" aria-current={pathname === "/pages/about" ? "page" : undefined}>About</a>
