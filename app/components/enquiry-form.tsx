...
export default function EnquiryForm() {
  ...
  useEffect(() => {
    ...
    if (requested === 'supply' || requested === 'design' || requested === 'file') setRoute(requested);
    let referrerHost = '';
  ...
  useEffect(() => {
    if (!config?.turnstileSiteKey || !challenge.current) return;
    let cancelled = false;
    const mount = () => {
  ...
  const send = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (state === 'sending' || !form.current) return;
    ...
    const payload = { submissionId: submissionId.current, route, website: field('website'), consent: values.get('consent') === 'on',
      ...
      brief: field('brief'), quantity: field('quantity'), annualQuantity: field('annualQuantity'), frequency: field('frequency'),
    ...
    if (!checked.data || Object.keys(checked.errors).length) return;
  ...
  return <form ref={form} className="enquiry-form" onSubmit={send} onChange={() => { if (!started.current) { started.current = true; track('enquiry_started', route); } }} noValidate>
    ...
    <fieldset disabled={state === 'sending'}><legend>1. Your project</legend>
      <label>Enquiry type<select name="route" value={route} onChange={event => setRoute(event.target.value as EnquiryRoute)}><option value="bulk">Bulk / event / trade order</option><option value="supply">Repeat orders / ongoing supply</option><option value="design">Help with a custom design</option><option value="file">Part / prototype / 3D file</option></select></label>
      <label>What would you like to make? <span>(required)</span><textarea name="brief" maxLength={5000} rows={4} {...props('brief')} placeholder="What is it, who is it for, and what does it need to do?"/>{error('brief')}</label>
      ...
      {route === 'supply' && <div className="enquiry-grid"><label>Estimated annual quantity<input name="annualQuantity" type="number" min="1" max={10000000} step="1" {...props('annualQuantity')}/>{error('annualQuantity')}</label><label>Order frequency<input name="frequency" maxLength={200} placeholder="e.g. monthly, quarterly, seasonal"/></label></div>}
    ...
    <fieldset disabled={state === 'sending'}><legend>2. Timing &amp; delivery</legend><div className="enquiry-grid"><label>Required delivery date<input name="requiredBy" type="date" {...props('requiredBy')}/>{error('requiredBy')}</label><label>Delivery suburb / postcode<input name="postcode" maxLength={100} autoComplete="postal-code"/></label></div><label className="check-label"><input type="checkbox" name="deadlineFixed"/>This delivery date is fixed</label><label>Multiple destinations or delivery notes <span>(optional)</span><input name="destinations" maxLength={500}/></label></fieldset>
    <fieldset disabled={state === 'sending'}><legend>3. Your contact details</legend><div className="enquiry-grid"><label>Your name <span>(required)</span><input name="name" autoComplete="name" maxLength={200} {...props('name')}/>{error('name')}</label><label>Email <span>(required)</span><input name="email" type="email" autoComplete="email" maxLength={254} {...props('email')}/>{error('email')}</label><label>Company / organisation<input name="company" autoComplete="organization" maxLength={200}/></label><label>Phone <span>(optional)</span><input name="phone" type="tel" autoComplete="tel" maxLength={40}/></label></div>
