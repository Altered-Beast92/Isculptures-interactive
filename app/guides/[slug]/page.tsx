...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main id="main-content" className="document-page guide-page"><article>
    <Breadcrumbs items={[{ name: 'Guides', path: '/guides' }, { name: guide.title, path: '/guides/' + slug }]}/><p className="section-tag">{guide.category}</p><h1>{guide.title}</h1><p className="document-lede">{guide.summary}</p><p className="guide-byline">From the Impeccable Sculptures studio</p>
