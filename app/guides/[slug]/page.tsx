...
import { CONTACT, SITE_URL, pageMetadata, CONTENT_DATES, displayDate } from '../../../lib/site';
...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const published = guide.published ?? CONTENT_DATES.guides.published;
  const updated = guide.updated ?? CONTENT_DATES.guides.updated;
  const schema = { '@context': 'https://schema.org', '@type': 'Article', headline: guide.title, description: guide.summary, mainEntityOfPage: SITE_URL + '/guides/' + slug, datePublished: published, dateModified: updated, image: SITE_URL + photo.src, author: { '@type': 'Organization', '@id': SITE_URL + '/#organisation', name: CONTACT.name }, publisher: { '@id': SITE_URL + '/#organisation' } };
  return <main id="main-content" className="document-page guide-page"><article>
    <Breadcrumbs items={[{ name: 'Guides', path: '/guides' }, { name: guide.title, path: '/guides/' + slug }]}/><p className="section-tag">{guide.category}</p><h1>{guide.title}</h1><p className="document-lede">{guide.summary}</p><p className="guide-byline">Written by Impeccable Sculptures &middot; <time dateTime={updated}>Updated {displayDate(updated)}</time></p>
