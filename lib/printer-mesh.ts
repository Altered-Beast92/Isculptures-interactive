// ISM1 is generated from the original GLB by scripts/prepare-printer-model.mjs.
...
export function parsePrinterMesh(buffer: ArrayBuffer) {
  ...
  if (header.getUint32(0, true) !== 0x314d5349 || vertices === 0 || vertices > 65535 || indices === 0 || indices % 3 !== 0 || buffer.byteLength !== 76 + vertices * 12 + indices * 2) {
    throw new Error('Invalid printer mesh');
  }
  return {
    matrix: new Float32Array(buffer, 12, 16),
    positions: new Float32Array(buffer, 76, vertices * 3),
    indices: new Uint16Array(buffer, 76 + vertices * 12, indices),
  };
}
